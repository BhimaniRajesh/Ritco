    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (String.Compare(strMethod, "GetServiceTax", StringComparison.Ordinal) == 0)
        {
            ...
            if (!string.IsNullOrEmpty(parameter1))
            {
                ...
                var hSBCessRate = Convert.ToString(TaxRateController.GetSwachhBharatCessRate(strDocumentDate));
                var hKKCessRate = Convert.ToString(TaxRateController.GetKrushiKalyanCessRate(strDocumentDate));

                Response.Clear();
                Response.Write("true|" + serviceTaxRate + "|" + eduCessRate + "|" + hEduCessRate + "|" + hSBCessRate + "|" + hKKCessRate + "|");
            }
            else
            {
                Response.Clear();
