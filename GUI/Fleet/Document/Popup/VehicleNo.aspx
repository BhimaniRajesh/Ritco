    ...
    <link id="Link1" href="~/GUI/images/style.css" rel="Stylesheet" type="text/css" runat="server" />
     <script type="text/javascript" language="javascript">
         ...
         function selectVehno(obj) 
         {
            ...
            var hf_Opener_VehType = document.getElementById("hVehType").value 
            ...
            window.opener.document.getElementById(hf_Opener_Km).value=returnValue[2];
            window.opener.document.getElementById(hf_Opener_VehType).value=returnValue[3];
			window.close();
        }
    ...
    <form id="form1" runat="server">
       ...
       <%--     <asp:HiddenField ID="hVehCategory" runat="server" Value=""/>--%>
            ...
            <asp:HiddenField ID="hKm" runat="server" Value=""/>
                       <asp:HiddenField ID="hVehType" runat="server" Value=""/>
           </tr>
                <tr style="background-color: white">
                    <td align="left" style="height: 56px">
                        ...
                        <asp:GridView Width="100%" HeaderStyle-CssClass="dgHeaderStyle" CssClass="dgRowStyle" BorderColor="#8BA0E5"
                            ...
                            <Columns>
                                <asp:TemplateField ShowHeader="False">
                                    <ItemTemplate>  
                                        <input type="radio" name="radSelect" onclick="javascript:selectVehno(this);" value='<%#Eval("VEHNO")%>~<%#Eval("VEH_INTERNAL_NO")%>~<%#Eval("CURRENT_KM_READ")%>~<%#Eval("Type_Name")%>' />
                                    </ItemTemplate>
                        ...
                        <asp:SqlDataSource ID="SqlConn" runat="server" ProviderName="System.Data.SqlClient"
                            SelectCommand="select distinct VEHNO,VEH_INTERNAL_NO,VH.CURRENT_KM_READ,vt.Type_Name
                                                    from webx_vehicle_type VT inner join
