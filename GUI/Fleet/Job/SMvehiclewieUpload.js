...
function CheckValid()
{
    var FileUpload_SM = document.getElementById("ctl00_MyCPH1_FileUpload_SM"); 
    var lbl_Error_Msg = document.getElementById("ctl00_MyCPH1_lbl_Error_Msg"); 
    
    if(FileUpload_SM.value == "")
    {
        lbl_Error_Msg.innerText = "Select File.";
        FileUpload_SM.style.borderColor="red";
        a = false;
        return false;
    }
    if(a==false)
...
function getSize(FileUpload_SM)
{
    var oas = new ActiveXObject("Scripting.FileSystemObject");
    var d = FileUpload_SM.value;
    var e = oas.getFile(d);
    var f = e.size;
    return(f);
}
function ValidFile(FileUpload_SM)
{
    var lbl_Error_Msg = document.getElementById(controlIdPrefix+"lbl_Error_Msg"); 
    if(FileUpload_SM.value!="")
    {
        var path = FileUpload_SM.value;
        ...
        if ((filetype.toLowerCase() == ".xls") ||(filetype.toLowerCase() == ".xlsx"))
        {
            ...
            FileUpload_SM.style.borderColor="green";
            a = true;
            var x = getSize(FileUpload_SM); 
            if (x > 1048576)
            {
