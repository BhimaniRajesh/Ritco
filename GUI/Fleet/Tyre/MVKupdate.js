...
function CheckValid()
{
    var FileUpload_MVK = document.getElementById("ctl00_MyCPH1_FileUpload_MVK"); 
    var lbl_Error_Msg = document.getElementById("ctl00_MyCPH1_lbl_Error_Msg"); 
    
    if(FileUpload_MVK.value == "")
    {
        lbl_Error_Msg.innerText = "Select File.";
        FileUpload_MVK.style.borderColor="red";
        a = false;
        return false;
    }
    if(a==false)
...
function getSize(FileUpload_MVK)
{
    var oas = new ActiveXObject("Scripting.FileSystemObject");
    var d = FileUpload_MVK.value;
    var e = oas.getFile(d);
    var f = e.size;
    return(f);
}
function ValidFile(FileUpload_MVK)
{
    var lbl_Error_Msg = document.getElementById(controlIdPrefix+"lbl_Error_Msg"); 
    if(FileUpload_MVK.value!="")
    {
        var path = FileUpload_MVK.value;
        ...
        if ((filetype.toLowerCase() == ".xls") ||(filetype.toLowerCase() == ".xlsx"))
        {
            ...
            FileUpload_MVK.style.borderColor="green";
            a = true;
            var x = getSize(FileUpload_MVK); 
            if (x > 1048576)
            {
