<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/GUI/MasterPage.master"  CodeFile="ExpenseVoucherAdd.aspx.cs" Inherits="GUI_Fleet_Operations_Expense_ExpenseVoucherAdd" %>

<asp:Content ID="Content1" ContentPlaceHolderID="MyCPH1" runat="Server">

<%

    string ExpMode = Request.QueryString["ExpMode"];
    ...
    for (intIndex = 2; intIndex <= document.getElementById("ctl00_MyCPH1_dgExpense").rows.length; intIndex++)
 {
    ...
    window.open("wfrm_Popup_Vehicle.aspx?Selected_VehNo=" + selected_VehNo + "&Opener_tb_SKU_Code=" + txtVehNo.id , null, "height=300,width=400,left=300,top=200,status=no,resizable=yes,scrollbars=yes,toolbar=no,location=no,menubar=no");
        ...
        function CheckValid()
        {
            ...
            if(mStat=="S")
            {
                grid1=document.getElementById("ctl00_MyCPH1_dgExpense1");
                 r1=grid1.rows.length;
            }
            
            if(mCrewSal=="C")
            {
                grid2=document.getElementById("ctl00_MyCPH1_dgExpense2");
                 r2=grid2.rows.length;
            }
            
            if(mVehTrack=="V")
            {
                grid3=document.getElementById("ctl00_MyCPH1_dgExpense3");
                 r3=grid3.rows.length;
             }
            if(mFinExp=="F")
            { 
                grid4=document.getElementById("ctl00_MyCPH1_dgExpense4");
                r4=grid4.rows.length;
            }
           
            if(mRepair=="R")
            { 
                grid5=document.getElementById("ctl00_MyCPH1_dgExpense");
...
<asp:UpdatePanel ID="UpdatePanel10" runat="server" RenderMode="Inline" UpdateMode="Conditional">
    <ContentTemplate>
       ...
       <table Width="80%"   cellspacing="1" cellpadding="3" class="boxbg" border="0">
            <tr style="background-color: white">
                    <td  width="37%"><%--width="237px"--%>
                        <font class=blackfnt><b>Total</b></font>
                    </td>
                    <td align="left" width="31%"><%--width="400px"--%>
