                ...
                if (xmlHttpRequest) {
                    xmlHttpRequest.onreadystatechange = function () {
                        if (xmlHttpRequest.readyState == 4) {
                            var returnValue = xmlHttpRequest.responseText.split("~");
                            if (returnValue[0] == "N") {
                                alert("Invalid Customer. OR Customer Is not Defined In Customer Master !!! Please check.");
                    ...
                    var currentTime = new Date()
                    xmlHttpRequest.open("GET", "AJAX_Validator/AjaxResponse.aspx?Function=CheckValidCustomer&datetime=" + currentTime + "&Customer=" + CustomerCode[1].trim(), false);
                    xmlHttpRequest.send(null);
                }
            }
            else {
...
function GetBalanceAmountDetailsM(TextBoxBalanceAmountM) {
    var gvVehicleLogEntry = document.getElementById("ctl00_MyCPH1_gvVehicleLogEntry");
    if (gvVehicleLogEntry != null) {
        //alert("1");
        if (gvVehicleLogEntry.rows.length != 0) {
        ...
        if (gvVehicleLogEntry.rows.length != 0) {
            for (i = 0; i < gvVehicleLogEntry.rows.length - 1; i++) {
                ...
                if (TexLoadingDateM.value != "") {
                    ...
                    //Added by Sandeep Maurya to validate  date which is not future date related to error RIT_005630
                    if (!IsCompareDateTense(TexLoadingDateM.value, CurrentDate, 'PastNPresent')) {
                        TexLoadingDateM.style.borderColor = "red";
                        lbl_VehicleLog_Err.innerText = "Loading Date Should Be Less Than Or Equals To Current Date!!!";
                        TexLoadingDateM.focus();
                        return false;
                    }
                    else {
        ...
        if (gvVehicleLogEntry.rows.length != 0) {
            for (i = 0; i < gvVehicleLogEntry.rows.length - 1; i++) {
                ...
                if (ddlAdvanceTakenBy.value == "3") {
                    ...
                    if (parseFloat((parseFloat(TextBoxAdvanceAmountByDriverM.value) + parseFloat(TextBoxAdvanceAmountByBranchM.value))) != parseFloat(TextBoxAdvanceAmountM.value)) {
                        alert("Total advance amount not matching with advance taken by driver amount/advance taken by branch amount");
                        TextBoxAdvanceAmountByDriverM.focus();
                        return false;
                    }
                }
