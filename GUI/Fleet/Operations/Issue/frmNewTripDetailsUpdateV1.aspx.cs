...
public partial class GUI_Fleet_Operations_Issue_frmNewTripDetailsUpdateV1 : System.Web.UI.Page
{
    static string str_Connection_String = "";
    public string clintName = "", path = "UploadedImages";
    public string modeval = "";
    public MyFleetDataSet _dataSet = new MyFleetDataSet();
    MyFleetDataSet.ETMRV1Row datarow_ETMRV1;
    string upload_file;
            ...
            try
            {
                ...
                foreach (GridViewRow gridrow in gvVehicleLogEntry.Rows)
                {
                    if (((Label)gridrow.FindControl("LabelExternalTHCNoM")).Text.ToString().Trim() == "")
                    {
                        ...
                        rTSmCityC.Close();
                        SqlCommand sql_insert_ExternalTHC = new SqlCommand("USP_WEBX_FLEET_ExternalTHC_ExternalTHC_Ver1", conn, trans);
                    ...
                    else
                    {
                        ...
                        rTSmCityC.Close();
                        SqlCommand sql_insert_ExternalTHC = new SqlCommand("USP_WEBX_FLEET_ExternalTHC_ExternalTHC_Update_Ver1", conn, trans);
                        sql_insert_ExternalTHC.CommandType = CommandType.StoredProcedure;
    ...
    protected void Add_RowVehicleLog(object sender, EventArgs e)
    {
        ...
        if (mroww >= count)
        {
            ...
            if (iroww >= 0)
            {
                ...
                {
                    _dataSet.ETMRV1.RemoveETMRV1Row(_dataSet.ETMRV1[i]);
        ...
        gvVehicleLogEntry.DataSource = _dataSet.ETMRV1;
    ...
    protected void gvVehicleLogEntry1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow || e.Row.RowType == DataControlRowType.EmptyDataRow)
        {
            ...
            Label lblUloadLink = (Label)e.Row.FindControl("lblUloadLink");
            if (_dataSet.ETMRV1.Count > 0)
            {
                ddlCustomerType.SelectedValue = _dataSet.ETMRV1[e.Row.RowIndex]["ddlCustomerType"].ToString();
                ddlAdvanceTakenBy.SelectedValue = _dataSet.ETMRV1[e.Row.RowIndex]["ddlAdvanceTakenBy"].ToString();
                if (_dataSet.ETMRV1[e.Row.RowIndex]["lblUloadLink"].ToString() != "")
                {
                    fUThcDoc.Visible = false;
                    lblUloadLink.Visible = true;
                }
                if (_dataSet.ETMRV1[e.Row.RowIndex]["LabelExternalTHCNoM"].ToString() != "")
                {


                    TextBoxUserIdM.Enabled = false;
                    TextBoxVehicleNoM.Enabled = false;
                    TextBoxTripsheetNoM.Enabled = false;
                    ...
                    if (lblEndLoc.Text.ToString() == SessionUtilities.CurrentBranchCode.ToString().Trim() && _dataSet.ETMRV1[e.Row.RowIndex]["lblUloadLink"].ToString() == "")
            ...
            TextBoxBalancePaymentM.Text = lblEndLoc.Text.Trim();
