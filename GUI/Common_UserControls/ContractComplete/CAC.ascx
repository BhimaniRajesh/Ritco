<%@ Control Language="C#" AutoEventWireup="true" CodeFile="CAC.ascx.cs" Inherits="ContractComplete" %>
...
<script type="text/javascript" language="javascript">
    ...
    // Declaration
    function dacItemClick(cval, nval, t_val3, target1id, target2id, target3id, divid) {
        ...
        var target3 = document.getElementById(target3id);
    ...
    function GetDACObj(idd, nmtxtid, tg3id, entityid, filt1id, filt2id, fltr_str, divxacid, inoid, minlen, c_idid, fs_stid, ms_fsid) {
        ...
        var tp = document.getElementById(entityid);
        WebXObj.WebxComplete.Type = tp.value;
    ...
    function dacKeyDown(e, WEBX) {
        ...
        if (code == 40) {
            ...
            if (cval) WEBX.WebxComplete.Target1.value = cval.innerHTML;
            ...
            event.returnValue = false;
            event.cancel = true; e.keyCode = 0;
        }
        return false;
    }

    function dacCodeKeyPress(e, WEBX, int) {
    ...
    function dacCodeKeyUp(e, WEBX, init) {
        ...
        if (code == 40) {
        ...
        else if (code == 38 || code == 37 || code == 39 || code == 13) {
            ...
            if (findobj) {
                ...
                var tp
                var strpg = fileURL.substring(0, cnt) + '/GUI/Common_UserControls/ContractComplete/_cac_data.aspx?tp=' + WEBX.WebxComplete.Type + '&target1=' + WEBX.WebxComplete.Target1.getAttribute('id');
    ...
    function dacCodeFocus(initid) {
        var init = document.getElementById(initid);
        init.value = "1";
    }
    ...
    function dacItemMouseOver(tcid) {
        var tc = document.getElementById(tcid);
        tc.setAttribute("font-weight", "bold");
        tc.style.fontWeight = 'bold';
        tc.setAttribute("bgColor", "yellow");
        tc.style.cursor = 'pointer';
    }

    function dacItemMouseOut(tcid) {
        var tc = document.getElementById(tcid);
        tc.setAttribute("font-weight", "normal");
        tc.style.fontWeight = 'normal';
        tc.setAttribute("bgColor", "white");
    }
</script>
...
<asp:HiddenField ID="tp" Value="0" runat="server" />
