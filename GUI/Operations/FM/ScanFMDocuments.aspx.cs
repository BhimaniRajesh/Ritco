...
public partial class GUI_Operations_FM_ScanFMDocuments : System.Web.UI.Page
{
    ...
    protected void txtDocketNo_TextChanged(object sender, EventArgs e)
    {
        ...
        if (txtdoc.Text == "")
        {
            ...
            lblDocketNo.Text = "Invalid Docket No.! or Not Delivered at " + brcd;
            return;
        }
        ...
        try
        {
            ...
            foreach (DataGridItem gridrow in myGrid.Items)
            {
                ...
                try
                {
                    ...
                    string docval = "", docval1 = "",DocumentUploadedPath="";
                    bool IsValid = false;
                    ...
                    if (IsValid)
                    {
                        ...
                        {                           
                            ...
                            DocumentUploadedPath = AzureStorageHelper.UploadBlob(dll_doctyp.SelectedItem.Text, upload_file, newFName);
                            if (!string.IsNullOrEmpty(DocumentUploadedPath))
                            {
                                ///Commented by Akash - POD will upload on Azure not required to save POD on local file system
                                ...
                                string SubmitData = "exec " + sp + " '" + dll_doctyp.SelectedValue + "','" + txtdockno.Text + "','" + txtdoc_no.Text + "','0','" + DocumentUploadedPath + "',''";
                                //SqlCommand cmd_subdata = new SqlCommand(SubmitData, cn);
                                //cmd_subdata.Transaction = trans;
                                //cmd_subdata.ExecuteNonQuery();
                                //if (trans != null)
                                //    trans.Commit();
                                SqlHelper.ExecuteNonQuery(SessionUtilities.ConnectionString, CommandType.Text, SubmitData);
                            }
                            else
                            {
                                //trans.Rollback();
                        ...
                        string st_filename = DocumentUploadedPath;// newFName;
