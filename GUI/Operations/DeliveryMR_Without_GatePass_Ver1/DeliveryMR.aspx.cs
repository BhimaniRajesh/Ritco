...
public partial class GUI_Operation_DeliveryMR_DeliveryMR : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            txtdockno.Text = Dockno;
            HdnMrLoc.Value = MrLocation;
        ...
        if (!IsPostBack)
        {
            fn.Fill_VendorName_Dataset_Gatepass();
    ...
    protected void btnstep1_Click(object sender, EventArgs e)
    {
        ...
        strsql = strsql + " dacc_yn,cod_dod,insuyn,G.CodeDesc AS packtype,CONVERT(VARCHAR,cdeldt,106) AS edd,chrgwt,convert(varchar,arrived_dt,106) as ArrivedDate,";
        strsql = strsql + " prodcd=ISNULL(GP.CodeID,'08'),csgecd,csgenm,stax_exmpt_yn,CONVERT(INT,pkgsno) AS totpkgs,stax_paidby,curr_loc,invamt=ISNULL(invamt,0),product=ISNULL(GP.codedesc,prodcd),";
        strsql = strsql + " godown=(SELECT godown_desc AS godownname FROM dbo.webx_godownmst WITH(NOLOCK) WHERE godown_srno=s.godown),isNULL(arrpkgqty,d.PKGSNO) as arrpkgqty,isNULL(arrweightqty,d.ACTUWT) as arrweightqty,PKGSTY";
        ...
        strsql = strsql + " LEFT OUTER JOIN dbo.webx_master_general G WITH(NOLOCK) ON G.CodeType='PKGS' AND G.codeid=pkgsty";
        ...
        hdnPKGSTY.Value = dt_docket.Rows[0]["PKGSTY"].ToString();
        strsql = "select ptcd=isnull(g.ptcd,''),ptname=isnull(g.ptname,'') from webx_trans_docket_status s ";
        strsql = strsql + " inner join webx_gatepass g on s.dockno=g.dockno ";
        strsql = strsql + " where g.dockno='" + txtdockno.Text + "' and docksf='" + ddlsuffix.SelectedValue + "' and isnull(s.gatepassno,'')<>'' ";

        DataTable Dt_Gatepass_Party = SqlHelper.ExecuteDataset(SessionUtilities.ConnectionString, CommandType.Text, strsql).Tables[0];

        string Party_Code = "", Party_Name = "";

        Party_Code = dt_docket.Rows[0]["csgecd"].ToString();
        Party_Name = dt_docket.Rows[0]["csgenm"].ToString();

        if (Dt_Gatepass_Party.Rows.Count > 0)
        {
            Party_Code = Dt_Gatepass_Party.Rows[0]["ptcd"].ToString();
            Party_Name = Dt_Gatepass_Party.Rows[0]["ptname"].ToString();
        }
        if (hdnpaybas.Value.CompareTo("P02") == 0)
        {
            ...
            if (Party_Code.CompareTo("8888") == 0)
            {
                txtpartyw.Text = Party_Name;
            }
            else
            {
                txtpartycode.Text = Party_Code;
                txtpartyname.Text = DataProvider.GetCustomerName(Party_Code);
            }
    ...
    {
        CalculateOutstanding();
        ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "", "initializer();", true);
    }

    protected void CalculateOutstanding()
    {
        DataTable TotalOutstanding = new DataTable();
        DataTable PendingOutstanding = new DataTable();
        DataTable DetailOthers = new DataTable();
        DataTable CreditDaysDT = new DataTable();
        ...
        string Outstanding = "", PendOutstanding = "", Details = "", CreditDays = "";

        try
        {
            if (optpartyw.Checked == true)
                TxtTotalOutstanding.Text = "0.00";
            else
            {
                Details = "EXEC USP_GET_COMMITED_BUSINESS_DELMR '" + hdncontractid.Value + "','" + txtpartycode.Text + "'";
                DetailOthers = SqlHelper.ExecuteDataset(SessionUtilities.ConnectionString, CommandType.Text, Details).Tables[0];
                if (DetailOthers.Rows.Count > 0)
                {
                    LblCommittedBusiness.Text = DetailOthers.Rows[0]["COMMITEDBUSINESS"].ToString();
                    LblCreditType.Text = DetailOthers.Rows[0]["CreditType"].ToString();
                }
                else
                {
                    LblCommittedBusiness.Text = "0.00";
                    LblCreditType.Text = "";
                }

                Outstanding = "EXEC USP_CUSTOMEROUTSTANDING_DELMR '" + txtpartycode.Text + "','" + SessionUtilities.FinYear + "','" + WebXConvert.ToDateTime(txtmrdate.Text, "en-GB").ToString("dd MMM yyyy") + "','" + SessionUtilities.CurrentBranchCode.ToString().Trim() + "','" + LblCreditType.Text.Trim() + "'";

                TotalOutstanding = SqlHelper.ExecuteDataset(SessionUtilities.ConnectionString, CommandType.Text, Outstanding).Tables[0];
                if (TotalOutstanding.Rows.Count > 0)
                    TxtTotalOutstanding.Text = TotalOutstanding.Rows[0]["AMT"].ToString();
                else
                    TxtTotalOutstanding.Text = "0.00";

                PendOutstanding = "EXEC Usp_CustOutStanding_DELMR '" + txtpartycode.Text + "','" + SessionUtilities.FinYear + "','" + WebXConvert.ToDateTime(txtmrdate.Text, "en-GB").ToString("dd MMM yyyy") + "','" + SessionUtilities.CurrentBranchCode.ToString().Trim() + "','" + LblCreditType.Text.Trim() + "'";

                PendingOutstanding = SqlHelper.ExecuteDataset(SessionUtilities.ConnectionString, CommandType.Text, PendOutstanding).Tables[0];
                if (PendingOutstanding.Rows.Count > 0)
                    TxtPendingOutstanding.Text = PendingOutstanding.Rows[0]["PENDAMT"].ToString();
                else
                    TxtPendingOutstanding.Text = "0.00";

                CreditDays = "EXEC USP_GET_CreditDay_Calculation_DELMR '" + WebXConvert.ToDateTime(hdndockdate.Value, "en-GB").ToString("dd MMM yyyy") + "','" + WebXConvert.ToDateTime(txtmrdate.Text, "en-GB").ToString("dd MMM yyyy") + "','" + txtpartycode.Text + "','" + LblCreditType.Text.Trim() + "'";
                CreditDaysDT = SqlHelper.ExecuteDataset(SessionUtilities.ConnectionString, CommandType.Text, CreditDays).Tables[0];
                if (CreditDaysDT.Rows.Count > 0)
                {
                    if (Convert.ToDouble(CreditDaysDT.Rows[0]["EntryDays"].ToString())<0)
                    {
                        if (Convert.ToDouble(TxtPendingOutstanding.Text) > 0)
                        {
                            btnsubmit.Enabled = false;
                            throw new Exception("You Are Not Able to Generate Delivery MR Due to Non-Payment " + TxtPendingOutstanding.Text + "");
                        }
                    }
                    else
                    {
                        ...
                        {
                            if (Convert.ToDouble(TxtPendingOutstanding.Text) > 0)
                                throw new Exception("Please Pay your Due’s “" + TxtPendingOutstanding.Text + "” Before “Today EOD” Other Wise Delivery MR will not Generate");
                        }
                        else
                        {
                            if (Convert.ToDouble(TxtPendingOutstanding.Text) > 0)
                                throw new Exception("Please Pay your Due’s “" + TxtPendingOutstanding.Text + "” Before “" + CreditDaysDT.Rows[0]["EntryDays"].ToString() + "” Days Other Wise Delivery MR will not Generate");
                        }
                    }
                }
            }
    ...
    protected void btnstep2_Click(object sender, EventArgs e)
    {
       ...
       ******************************************************************************************************/
        ...
        hdncontractid.Value = DocketController.GetContractID(txtpartycode.Text, hdnpaybas.Value, WebXConvert.ToDateTime(hdndockdate.Value, "en-GB").ToString("dd MMM yyyy"));
        ...
        if (("ABCDEFGHIMXYZ".Contains(ddlsuffix.SelectedValue) && (WebXConvert.ToInt16(hdnarrpkgqty.Value) <= WebXConvert.ToInt16(hdnpkgsno.Value)) || (ddlsuffix.SelectedValue.CompareTo(".") == 0 && WebXConvert.ToInt16(hdnarrpkgqty.Value) > 0)))
        {
            if (GetContractInvokeFlag() && hdncontractid.Value.CompareTo("") != 0)
            {
                ...
                ckeys.DockDt = WebXConvert.ToDateTime(txtmrdate.Text, "en-GB");
                ...
                HdnDefaultContractId.Value = ckeys.ContractID;
    ...
    protected void loadBookingCharges()
    {
        ...
        HdnFreightAmount.Value = dt_charges.Rows[0]["Freight"].ToString();
        HdnSubtotalAmount.Value = dt_charges.Rows[0]["Subtotal"].ToString();
    ...
    protected void loadDeliveryCharges()
    {
        ...
        ***********************************************************************************************/
        ...
        HiddenField Hdndiffsubtotal = (HiddenField)grvcharges.FooterRow.FindControl("Hdndiffsubtotal");
        dtr["chargevalue"] = Hdndiffsubtotal.Value.CompareTo("") == 0 ? "0" : Hdndiffsubtotal.Value;
    ...
    protected void calculateFreightDifference()
    {
        ...
        double freightdiff = 0, ActualFreightDiff = 0, bkgcharge = 0, delcharge = 0;
        TextBox txtdiffsubtotal = (TextBox)grvcharges.FooterRow.FindControl("txtdiffsubtotal");
        HiddenField Hdndiffsubtotal = (HiddenField)grvcharges.FooterRow.FindControl("Hdndiffsubtotal");

        if (HdnDefaultContractId.Value == "P038888")
        {
            txtdelsubtotal.Text = HdnSubtotalAmount.Value;
            txtdiffsubtotal.Text = HdnSubtotalAmount.Value;
            Hdndiffsubtotal.Value = "0.00";
        }
        else
        {
            foreach (GridViewRow grv in grvcharges.Rows)
            {
                ...
                double diff = 0, ActualDiff = 0;
                if (grv.RowIndex != 0 && grv.RowIndex != 1)
                {
                    ActualDiff = Convert.ToDouble(txtdelcharge.Text) - Convert.ToDouble(lblbkgcharge.Text);
                    bkgcharge = bkgcharge + Convert.ToDouble(lblbkgcharge.Text);
                    delcharge = delcharge + Convert.ToDouble(txtdelcharge.Text);
                    diff = Convert.ToDouble(txtdelcharge.Text);
                    txtdiffcharge.Text = diff.ToString("F2");
                    freightdiff = freightdiff + diff;
                    //ActualFreightDiff = ActualFreightDiff + ActualDiff;
                }
            }
            if (hdnflagroundoff.Value.CompareTo("Y") == 0)
            {
                ActualFreightDiff = Math.Round(delcharge, 0, MidpointRounding.AwayFromZero) - Math.Round(bkgcharge, 0, MidpointRounding.AwayFromZero);
            }
            else
            {
                ActualFreightDiff = delcharge - bkgcharge;
            }
            //TextBox txtdiffsubtotal = (TextBox)grvcharges.FooterRow.FindControl("txtdiffsubtotal");
            //HiddenField Hdndiffsubtotal = (HiddenField)grvcharges.FooterRow.FindControl("Hdndiffsubtotal");
            if (hdnflagroundoff.Value.CompareTo("Y") == 0)
            {
                ...
                ActualFreightDiff = Math.Round(ActualFreightDiff, 0, MidpointRounding.AwayFromZero);
            }
            txtdiffsubtotal.Text = freightdiff.ToString("F2");
            Hdndiffsubtotal.Value = ActualFreightDiff.ToString("F2");
        }
    }
    protected void calculateSubTotal()
    {
        ...
        foreach (GridViewRow grv in grvdelcharges.Rows)
        {
            ...
            if (RdbDDLChg.SelectedValue.ToString() == "N" && RdbFOC.SelectedValue.ToString() == "N" && hdnchargecode.Value == "SCHG08")
            {
                subtotal = subtotal + 0;
            }
            else
    ...
    protected void btnsubmit_Click(object sender, EventArgs e)
    {
        ...
        txtmramount.Text = hdnmramount.Value;
        txtcollamt.Text = hdncollamt.Value;
        
        txtservicetax.Text = hdnservicetax.Value;
        txtSBCTax.Text = hdnSBCTax.Value;
        txtKKCTax.Text = hdnKKCTax.Value;
        ...
        try
        {
            MRController.IsDocketEligibleForSeperateDelMR(txtdockno.Text.Trim(), ddlsuffix.SelectedValue, SessionUtilities.CurrentBranchCode, hdnfinstartyear.Value, trn);
        }
        catch (Exception ex)
        {
            trn.Rollback();
        ...
        Response.Redirect("DeliveryMR_Submit_Done.aspx?Location=" + HdnMrLoc.Value + "&mrno=" + dtmr.Rows[0]["mrno"].ToString() + "&billno=" + (Convert.ToString(dtmr.Rows[0]["billno"]).CompareTo("") == 0 ? "NA" : Convert.ToString(dtmr.Rows[0]["billno"])), false);
    }
    ...
    protected void calculateServiceTax()
    {
        ...
        if (hdncontractid.Value != "")
        {
            string sql = "SELECT top 1 stax_paidby FROM WEBX_CUSTCONTRACT_HDR WHERE ContractId='" + hdncontractid.Value + "' and activeflag='Y'";
            hdnstaxpayer.Value = SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, sql).ToString();
        }
        ...
        lblKKCRate.Text = dst.KKCessRate.ToString("F2");
        ...
        if (hdnflagstaxempted.Value.CompareTo("Y") == 0)
        {
            ...
            lblSBCRate.Text = "0.00";
            lblKKCRate.Text = "0.00";
        }
        ...
        if (hdnflagroundoff.Value.CompareTo("Y") == 0)
        {
            ...
            dst.SBCess = Math.Round(dst.SBCess, 0, MidpointRounding.AwayFromZero);
            dst.KKCess = Math.Round(dst.KKCess, 0, MidpointRounding.AwayFromZero);
        }
        ...
        txtSBCTax.Text = dst.SBCess.ToString("F2");
        txtKKCTax.Text = dst.KKCess.ToString("F2");
        // Generating and Applying Docket Total
        mrtotal = subtotal + dst.ServiceTax + dst.EduCess + dst.HEduCess + dst.SBCess + dst.KKCess;
    ...
    protected void grvcharges_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (e.Row.RowIndex > 2)
            {
                string[] _param_othercharge_req = new string[15];
                ...
                _param_othercharge_req[14] = hdnPKGSTY.Value;
                if (GetContractInvokeFlag() && hdncontractid.Value.CompareTo("") != 0)
                {
    ...
    protected void grvdelcharges_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            HiddenField hdnactiveflag = (HiddenField)e.Row.FindControl("hdnactiveflag");
            HiddenField HdnRiskCharges = (HiddenField)e.Row.FindControl("HdnRiskCharges");
            HiddenField HdnDemCharges = (HiddenField)e.Row.FindControl("HdnDemCharges");
            HiddenField HdnDocCharges = (HiddenField)e.Row.FindControl("HdnDocCharges");
            HiddenField HdnMiscCharges = (HiddenField)e.Row.FindControl("HdnMiscCharges");
            HiddenField HdnDDCharges = (HiddenField)e.Row.FindControl("HdnDDCharges");
            HiddenField HdnFovCharges = (HiddenField)e.Row.FindControl("HdnFovCharges");
            HiddenField HdnSurCharges = (HiddenField)e.Row.FindControl("HdnSurCharges");
            HiddenField HdnCarCharges = (HiddenField)e.Row.FindControl("HdnCarCharges");
            ...
            {
                if (hdnchargecode.Value == "SCHG14")
                {
                    DateTime ArrivalDt = WebXConvert.ToDateTime(lblUnLoadingDate.Text, "en-GB");
                    DateTime BookingDate = WebXConvert.ToDateTime(hdndockdate.Value, "en-GB");
                    DateTime MRDate = WebXConvert.ToDateTime(txtmrdate.Text, "en-GB");
                    TimeSpan span = MRDate - BookingDate;
                    day = Convert.ToInt32(span.TotalDays);
                    string[] _param_othercharge_req = new string[16];
                    _param_othercharge_req[0] = hdnchargecode.Value;
                    ...
                    _param_othercharge_req[12] = hdnchargedweight.Value;
                    _param_othercharge_req[13] = hdnpkgsno.Value;
                    _param_othercharge_req[14] = day.ToString();
                    _param_othercharge_req[15] = hdnprodtype.Value;

                    txtdelcharge.Text = DocketController.GetDemurrageCharge(_param_othercharge_req).ToString("F2");
                    HdnDemCharges.Value = txtdelcharge.Text;
                }  //end
                else
                {
                    var strContractId = (hdncontractid.Value.CompareTo("") == 0 ? hdnpaybas.Value + "8888" : hdncontractid.Value);
                    string[] _param_othercharge_req = new string[15];
                    _param_othercharge_req[0] = hdnchargecode.Value;
                    _param_othercharge_req[1] = "DEL";
                    _param_othercharge_req[2] = hdnchargerule.Value;
                    ...
                    _param_othercharge_req[4] = "NONE";
                    _param_othercharge_req[5] = "NONE";
                    _param_othercharge_req[6] = strContractId;
                    _param_othercharge_req[7] = hdnfrom.Value;
                    _param_othercharge_req[8] = hdnto.Value;
                    _param_othercharge_req[9] = hdnorgnloc.Value;
                    _param_othercharge_req[10] = hdndelloc.Value;
                    ...
                    {
                        string strSQL = "SELECT DBO.UFN_GET_OTHERCHARGE('SCHG16','DEL','" + hdnchargerule.Value + "','" + hdnbasecode1.Value + "','NONE','NONE',";
                        strSQL += "'" + strContractId + "','" + hdnfrom.Value + "','" + hdnto.Value + "','" + hdnorgnloc.Value + "','" + hdndelloc.Value + "',";
                        strSQL += "'" + hdntransmode.Value + "','" + hdnchargedweight.Value + "','" + hdnpkgsno.Value + "','" + hdndeclval.Value + "',";
                        strSQL += "'0','')";

                        txtdelcharge.Text = (SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, strSQL)).ToString();
                        HdnRiskCharges.Value = txtdelcharge.Text;
                    }
                    else if (hdnchargecode.Value == "SCHG01")
                    {
                        string strSQL = "SELECT DBO.UFN_GET_OTHERCHARGE('SCHG01','DEL','" + hdnchargerule.Value + "','" + hdnbasecode1.Value + "','NONE','NONE',";
                        strSQL += "'" + strContractId + "','" + hdnfrom.Value + "','" + hdnto.Value + "','" + hdnorgnloc.Value + "','" + hdndelloc.Value + "',";
                        strSQL += "'" + hdntransmode.Value + "','" + hdnchargedweight.Value + "','" + hdnpkgsno.Value + "','" + hdndeclval.Value + "',";
                        strSQL += "'0','')";

                        txtdelcharge.Text = (SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, strSQL)).ToString();
                        HdnDocCharges.Value = txtdelcharge.Text;
                    }
                    else if (hdnchargecode.Value == "SCHG03")
                    {
                        string strSQL = "SELECT DBO.UFN_GET_OTHERCHARGE('SCHG03','DEL','" + hdnchargerule.Value + "','" + hdnbasecode1.Value + "','NONE','NONE',";
                        strSQL += "'" + strContractId + "','" + hdnfrom.Value + "','" + hdnto.Value + "','" + hdnorgnloc.Value + "','" + hdndelloc.Value + "',";
                        strSQL += "'" + hdntransmode.Value + "','" + hdnchargedweight.Value + "','" + hdnpkgsno.Value + "','" + hdndeclval.Value + "',";
                        strSQL += "'0','')";

                        txtdelcharge.Text = (SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, strSQL)).ToString();
                        HdnMiscCharges.Value = txtdelcharge.Text;
                    }
                    else if (hdnchargecode.Value == "SCHG08")
                    {
                        string strSQL = "SELECT DBO.UFN_GET_OTHERCHARGE('SCHG08','DEL','" + hdnchargerule.Value + "','" + hdnbasecode1.Value + "','NONE','NONE',";
                        strSQL += "'" + strContractId + "','" + hdnfrom.Value + "','" + hdnto.Value + "','" + hdnorgnloc.Value + "','" + hdndelloc.Value + "',";
                        strSQL += "'" + hdntransmode.Value + "','" + hdnchargedweight.Value + "','" + hdnpkgsno.Value + "','" + hdndeclval.Value + "',";
                        strSQL += "'0','')";

                        txtdelcharge.Text = (SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, strSQL)).ToString();
                        HdnDDCharges.Value = txtdelcharge.Text;
                    }
                    else if (hdnchargecode.Value == "SCHG11")
                    {
                        string strSQL = "SELECT DBO.UFN_GET_OTHERCHARGE('SCHG11','DEL','" + hdnchargerule.Value + "','" + hdnbasecode1.Value + "','NONE','NONE',";
                        strSQL += "'" + strContractId + "','" + hdnfrom.Value + "','" + hdnto.Value + "','" + hdnorgnloc.Value + "','" + hdndelloc.Value + "',";
                        strSQL += "'" + hdntransmode.Value + "','" + hdnchargedweight.Value + "','" + hdnpkgsno.Value + "','" + hdndeclval.Value + "',";
                        strSQL += "'0','')";

                        txtdelcharge.Text = (SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, strSQL)).ToString();
                        HdnFovCharges.Value = txtdelcharge.Text;
                    }
                    else if (hdnchargecode.Value == "SCHG17")
                    {
                        string strSQL = "SELECT DBO.UFN_GET_OTHERCHARGE('SCHG17','DEL','" + hdnchargerule.Value + "','" + hdnbasecode1.Value + "','NONE','NONE',";
                        strSQL += "'" + strContractId + "','" + hdnfrom.Value + "','" + hdnto.Value + "','" + hdnorgnloc.Value + "','" + hdndelloc.Value + "',";
                        strSQL += "'" + hdntransmode.Value + "','" + hdnchargedweight.Value + "','" + hdnpkgsno.Value + "','" + hdndeclval.Value + "',";
                        strSQL += "'0','')";

                        txtdelcharge.Text = (SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, strSQL)).ToString();
                        HdnSurCharges.Value = txtdelcharge.Text;
                    }
                    else if (hdnchargecode.Value == "UCHG08")
                    {
                        string strSQL = "SELECT DBO.UFN_GET_OTHERCHARGE('UCHG08','DEL','" + hdnchargerule.Value + "','" + hdnbasecode1.Value + "','NONE','NONE',";
                        strSQL += "'" + strContractId + "','" + hdnfrom.Value + "','" + hdnto.Value + "','" + hdnorgnloc.Value + "','" + hdndelloc.Value + "',";
                        strSQL += "'" + hdntransmode.Value + "','" + hdnchargedweight.Value + "','" + hdnpkgsno.Value + "','" + hdndeclval.Value + "',";
                        strSQL += "'0','')";

                        txtdelcharge.Text = (SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, strSQL)).ToString();
                        HdnCarCharges.Value = txtdelcharge.Text;
                    }
                    //else if (hdnchargecode.Value == "SCHG14")
                    //{
                    //    string Mrdate = WebXConvert.ToDateTime(txtmrdate.Text, "en-GB").ToString("dd MMM yyyy");
                    //    string sqlstr = "exec USP_GET_DEM_CHARGE '" + txtdockno.Text + "','.','" + Mrdate + "','" + strContractId + "'";
                    //    //Response.Write(sqlstr);

                            //    txtdelcharge.Text = SqlHelper.ExecuteScalar(SessionUtilities.ConnectionString, CommandType.Text, sqlstr).ToString();
                    //    //txtdelcharge.Enabled = false;
                    //    HdnDemCharges.Value = txtdelcharge.Text;
                    //}
                    else
    ...
    private string GetMRObject()
    {
        ...
        string[] Agent = txtVendorName.Text.ToString().Split('~');
        ...
        string strXMLMREntry = "<root><mr_entry>"
               ...
               + "<mrstype>12</mrstype>" + "<mrsbr>" + HdnMrLoc.Value + "</mrsbr>" +
        ...
        + "<CreditLimit>" + txtCreditLimit.Text + "</CreditLimit>"
        ...
        + "<col_sbctax>" + txtSBCTax.Text + "</col_sbctax><sbctax_rate>" + lblSBCRate.Text.Trim() + "</sbctax_rate>"
        + "<col_kkctax>" + txtKKCTax.Text + "</col_kkctax><kkctax_rate>" + lblKKCRate.Text.Trim() + "</kkctax_rate>"
        + "<col_h_cess>" + txtheducess.Text + "</col_h_cess><mrsamt>" + txtmramount.Text.Trim() + "</mrsamt>"
        + "<mrs_closed>Y</mrs_closed>"
        + "<mr_cancel>N</mr_cancel><mrcollbrcd>" + HdnMrLoc.Value + "</mrcollbrcd>"
        ...
        strXMLMREntry = strXMLMREntry + "<DocType>" + HdnDocType.Value + "</DocType>";
        strXMLMREntry = strXMLMREntry + "<DDLCharge_Y_N>" + RdbDDLChg.SelectedValue.ToString() + "</DDLCharge_Y_N>";
        strXMLMREntry = strXMLMREntry + "<FOC_Y_N>" + RdbFOC.SelectedValue.ToString() + "</FOC_Y_N>";
        strXMLMREntry = strXMLMREntry + "<GatePassBrcd>" + HdnGatePassLoc.Value + "</GatePassBrcd>";
        strXMLMREntry = strXMLMREntry + "<remark>" + txtremarks.Text + "</remark>";
        strXMLMREntry = strXMLMREntry + "<agent>" + Agent[1].ToString() + "</agent>";
    ...
    private string GetGatePassObject()
    {
        ...
        strXMLGPEntry = strXMLGPEntry + "<delvehicle></delvehicle>";
    ...
    private string GetBillObject()
    {
        ...
        + "<billsubbrcd>" + HdnMrLoc.Value + "</billsubbrcd>"
        + "<billcolbrcd>" + HdnMrLoc.Value + "</billcolbrcd>"
        + "<svrcamt>" + txtservicetax.Text + "</svrcamt>"
        + "<cessamt>" + txteducess.Text + "</cessamt>"
        + "<hedu_cess>" + txtheducess.Text + "</hedu_cess>"
        + "<col_sbctax>" + txtSBCTax.Text + "</col_sbctax>"
        + "<col_kkctax>" + txtKKCTax.Text + "</col_kkctax>";
        if (WebXConvert.ToDouble(txtservicetax.Text) == 0)
        {
            strXMLBillEntry = strXMLBillEntry + "<svctax_rate>0</svctax_rate>"
                ...
                + "<sbctax_rate>0</sbctax_rate>"
                + "<kkctax_rate>0</kkctax_rate>";
        }
        else
        {
            ...
            + "<sbctax_rate>" + lblheducessrate.Text + "</sbctax_rate>"
            + "<kkctax_rate>" + lblKKCRate.Text + "</kkctax_rate>";
        }

        strXMLBillEntry = strXMLBillEntry + "<businesstype>" + hdnbusinesstype.Value + "</businesstype>"
        + "<dockno>" + txtdockno.Text + "</dockno>"
        + "<docksf>" + ddlsuffix.SelectedValue + "</docksf>"
    ...
    private string GetMRChargesObject()
    {
        ...
        + "<HEDU_CESS>" + txtheducess.Text + "</HEDU_CESS>"
        + "<SBCTAX>" + txtSBCTax.Text + "</SBCTAX>"
        + "<KKCTAX>" + txtKKCTax.Text + "</KKCTAX>"
    ...
    {
        txtsubtotal.Text = hdnSubtotal.Value;
        txtoctproccharge.Text = hdnoctproccharge.Value;
        Exception ex;
        double subtotal = 0;
        foreach (GridViewRow grv in grvdelcharges.Rows)
        {
        ...
        {
            if (RdbDDLChg.SelectedValue.ToString() != "Y" && RdbFOC.SelectedValue.ToString() != "Y")
            {
    ...
    public DocketServiceTax GetServiceTax(DocketServiceTax dst)
    {
        ...
        if (dst.TransMode.CompareTo("2") == 0 || dst.TransMode.CompareTo("4") == 0)
        {
            if (dst.ServiceType.CompareTo("1") == 0 && dst.SubTotal > 750)
            {
                ...
                dst.SbcRate = trate.SbcRate * 0.25;
            }
    ...
    public DataTable DeliveryMREntry(string strXMLMREntry, string strXMLMRChargesEntry, string strXMLMRDiffEntry, string strXMLGatepassEntry, string strXMLBillEntry, string flagacc, SqlTransaction trn)
    {
        ...
        try
        {
            ...
            dt = SqlHelper.ExecuteDataset(trn, CommandType.StoredProcedure, "USP_SEPERATE_DELMR_ENTRY_Ver1", paramsToStore).Tables[0];
