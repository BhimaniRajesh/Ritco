...
public partial class GUI_Operation_DeliveryMR_DeliveryMR : System.Web.UI.Page
{
	...
	protected void CalculateOutstanding()
    {
        ...
        try
        {
            ...
            {
                Outstanding = "EXEC Usp_CustOutStanding_DELMR '" + WebXConvert.ToDateTime(hdnserverdate.Value, "en-GB").ToString("dd MMM yyyy") + "','" + txtpartycode.Text + "','" + SessionUtilities.FinYear + "'";
                ...
                if (TotalOutstanding.Rows.Count > 0)
                    TxtTotalOutstanding.Text = TotalOutstanding.Rows[0]["PENDAMT"].ToString();

                CreditDays = "EXEC USP_GET_CreditDay_Calculation_DELMR '" + WebXConvert.ToDateTime(hdnserverdate.Value, "en-GB").ToString("dd MMM yyyy") + "','" + txtpartycode.Text + "'";
                ...
                {
                    if (CreditDaysDT.Rows[0]["Result"].ToString() == "N")
                    {
                        LblCreditDays.Text = "0 kalam udhar";
                        btnstep2.Enabled = false;
                        btnsubmit.Enabled = false;
                        throw new Exception("Delivery MR Not Possible to process becuase your credit days is over");
                    }
                    else
                        LblCreditDays.Text = CreditDaysDT.Rows[0]["Result"].ToString();
                }
            }
        }
        catch (Exception ex)
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (hdnvarybyloc.Value.CompareTo("Y") == 0)
            {
                if (hdnchargecode.Value == "SCHG14")
                {
                    ...
                    TimeSpan span = MRDate - ArrivalDt;
