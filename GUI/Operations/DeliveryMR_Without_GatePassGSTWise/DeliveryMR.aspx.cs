...
public partial class GUI_Operation_DeliveryMRGSTWise_DeliveryMR : System.Web.UI.Page
{
    ...
    protected void calculateSubTotal()
    {
        ...
        //ucGSTTaxControl.SubTotals = subtotal.ToString("F2");
        //calculateServiceTax();

        string GSTPaymentPartyCode = "", GSTPaymentPartyName = "";
        if (hdnpaybas.Value.CompareTo("P02") == 0)
        {
            if (optpartytype.SelectedValue.CompareTo("P") == 0)
            {
                ...
                GSTPaymentPartyCode = hdnpartycode.Value;
                GSTPaymentPartyName = hdnpartyname.Value;
            }
            else
            {
                if (optpartyc.Checked == true)
                {
                    GSTPaymentPartyCode = txtpartycode.Text;
                    GSTPaymentPartyName = txtpartyname.Text;
                }
                else
                {
                    GSTPaymentPartyCode = "";
                    GSTPaymentPartyName = txtpartyw.Text;
                }
            }
        }
        else
        ...
        ucGSTTaxControl.GSTProductID = hdntransmode.Value;
        ucGSTTaxControl.GSTPayBaseId = hdnpaybas.Value;
        ucGSTTaxControl.CityId = hdnfrom.Value;
        ucGSTTaxControl.BookingBranch = hdnorgnloc.Value;
        ucGSTTaxControl.SubTotals = subtotal;
        ucGSTTaxControl.DocketNumber = txtdockno.Text;
        
		ucGSTTaxControl.DestinatonCity = hdnto.Value;
		ucGSTTaxControl.DestinationBranch = hdndelloc.Value;
		ucGSTTaxControl.PartyAs = "2";
        ucGSTTaxControl.BilledAtBranch = hdndelloc.Value;
		ucGSTTaxControl.StartGSTControl();
        //GST Wise
        ScriptManager.RegisterStartupScript(this, this.GetType(), "calculateTotalOnCallBack_LOAD" + DateTime.Now.Ticks.ToString(), "calculateTotalforDeliveryMr();", true);
        ucGSTTaxControl.DocketMode = GUI_Common_UserControls_GSTTaxControl.mDocketMode.DeliveryMR;
    }
    ...
    private string GetMRObject()
    {
        ...
        //+ "<col_h_cess>" + txtheducess.Text + "</col_h_cess><mrsamt>" + ucGSTTaxControl.DocketTotal.Trim() + "</mrsamt>"

        + "<svctax_rate>0</svctax_rate><cess_rate>0</cess_rate>"
        + "<h_cess_rate>0</h_cess_rate><col_frt>" + freightcharge + "</col_frt>"
        + "<col_stax>0</col_stax><col_cess>0</col_cess>"
        + "<col_sbctax>0</col_sbctax><sbctax_rate>0</sbctax_rate>"
        + "<col_kkctax>0</col_kkctax><kkctax_rate>0</kkctax_rate>"
        + "<col_h_cess>0</col_h_cess><mrsamt>" + ucGSTTaxControl.DocketTotal + "</mrsamt>"

        + "<GSTParty>" + ucGSTTaxControl.GSTPaymentPartyCode + "</GSTParty>"
        + "<GSTState>" + ucGSTTaxControl.GSTBillingStateCode + "</GSTState>"
        + "<GSTIN>" + ucGSTTaxControl.GSTIN + "</GSTIN>"
        + "<SACCategory>" + ucGSTTaxControl.SACCategory + "</SACCategory>"
        + "<GSTServiceType>" + ucGSTTaxControl.GSTServiceType + "</GSTServiceType>"
        + "<ISRCMApplicable>" + ucGSTTaxControl.ISRCMApplicable+ "</ISRCMApplicable>"
        + "<IsGSTPartyRegistered>" + ucGSTTaxControl.IsGSTPartyRegistered + "</IsGSTPartyRegistered>"
        + "<EntryFrom>GSTMR</EntryFrom>"
        + "<IsGSTDocumentScan>" + ucGSTTaxControl.IsGStDocumentScan + "</IsGSTDocumentScan>"
        + "<GSTRemark>" + ucGSTTaxControl.GSTRemark + "</GSTRemark>"

		+ "<SACCode>" + ucGSTTaxControl.SACCode+ "</SACCode>"
		+ "<GSTExempted>" + ucGSTTaxControl.GSTExempted+ "</GSTExempted>"
		+ "<GSTExemptionCategory>" + ucGSTTaxControl.GSTExemptionCategory+ "</GSTExemptionCategory>"
		+ "<PlaceOfSupply>" + ucGSTTaxControl.PlaceOfSupply+ "</PlaceOfSupply>"
        ...
        strXMLMREntry = strXMLMREntry + "<agent></agent>";
    ...
    {
        Bill bill = new Bill();
        //double billamt = WebXConvert.ToDouble(ucGSTTaxControl.DocketTotal) - WebXConvert.ToDouble(txtcollamt.Text);
        double billamt = WebXConvert.ToDouble(ucGSTTaxControl.DocketTotal) - WebXConvert.ToDouble(txtcollamt.Text);
        ...
        + "<billamt>" + ucGSTTaxControl.DocketTotal.ToString("F2") + "</billamt>"
        ...
        + "<GSTServiceType>" + ucGSTTaxControl.GSTServiceType + "</GSTServiceType>"
        + "<SACCategory>" + ucGSTTaxControl.SACCategory + "</SACCategory>"
        + "<GSTRate>" + ucGSTTaxControl.GSTRate + "</GSTRate>"
        + "<ISRCMApplicable>" + (ucGSTTaxControl.ISRCMApplicable == "true" ? "Y" : "N") + "</ISRCMApplicable>"
        + "<GSTIN>" + ucGSTTaxControl.GSTIN + "</GSTIN>"
        + "<GSTState>" + ucGSTTaxControl.GSTBillingStateCode + "</GSTState>"
        + "<GSTCharged>" + ucGSTTaxControl.GSTCharged + "</GSTCharged>"
        ...
        + "<svrcamt>0</svrcamt>"
        + "<cessamt>0</cessamt>"
        + "<hedu_cess>0</hedu_cess>"
        + "<col_sbctax>0</col_sbctax>"
        + "<col_kkctax>0</col_kkctax>";
    ...
    private string GetMRChargesObject()
    {
        ...
        //strXMLMrChargesEntry = strXMLMrChargesEntry + "<SUBTOTAL>" + ucGSTTaxControl.SubTotals + "</SUBTOTAL>"
        //+ "<SVCTAX>" + txtservicetax.Text + "</SVCTAX>"
        //+ "<CESS>" + txteducess.Text + "</CESS>"
        //+ "<HEDU_CESS>" + txtheducess.Text + "</HEDU_CESS>"
        //+ "<SBCTAX>" + txtSBCTax.Text + "</SBCTAX>"
        //+ "<KKCTAX>" + txtKKCTax.Text + "</KKCTAX>"
        //+ "<MRSAMT>" + ucGSTTaxControl.DocketTotal + "</MRSAMT>"
        ...
        + "<KKCTAX>0</KKCTAX>"
        + "<GSTRate>" + ucGSTTaxControl.GSTRate + "</GSTRate>"
        + "<GSTAmount>" + ucGSTTaxControl.GSTAmount + "</GSTAmount>"
        + "<GSTCharged>" + ucGSTTaxControl.GSTCharged + "</GSTCharged>"
        + "<MRSAMT>" + ucGSTTaxControl.DocketTotal + "</MRSAMT>"
        + "</mr_charges_entry></root>";
    ...
    protected bool validateSubmit()
    {
        ...
        if (WebXConvert.ToDouble(ucGSTTaxControl.SubTotals) != subtotal)
        {
        ...
        if (DocketController.ToDouble(ucGSTTaxControl.SubTotals) < 0)
        {
            ex = new Exception("SubTotal Can't be less than Zero");
            throw ex;
        }

        if (DocketController.ToDouble(ucGSTTaxControl.DocketTotal) < 0)
        {
    ...
    public DataTable DeliveryMREntry(string strXMLMREntry, string strXMLMRChargesEntry, string strXMLMRDiffEntry, string strXMLGatepassEntry, string strXMLBillEntry, string flagacc, SqlTransaction trn)
    {
        ...
        try
        {
            ...
            dt = SqlHelper.ExecuteDataset(trn, CommandType.StoredProcedure, "USP_SEPERATE_DELMR_ENTRY_GSTWise", paramsToStore).Tables[0];
...
//if (DocketController.ToDouble(txtcollamt.Text) < DocketController.ToDouble(ucGSTTaxControl.DocketTotal))
//{
//    strXMLBillEntry = GetBillObject();
