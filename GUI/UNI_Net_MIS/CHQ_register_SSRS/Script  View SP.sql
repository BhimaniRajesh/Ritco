...
BANK=isnull(acccode,'')+' : '+(select top 1 accdesc from webx_acctinfo where acccode=c.acccode),
Party=isnull(ptmsptcd,'')+' : '+isnull(ptmsptnm,''),brcd,voucherno,acccode,ptmsptcd,adjustamt=cast(isnull(adjustamt,0) as decimal(9,2)),
NotUsedAmt=cast((isnull(chqamt,0)-isnull(adjustamt,0)) as decimal(9,2)),DepoFlag,OwnCust
...
select @WhereCri_DT='',@Where_Bank='',@Where_CHQ='',@WhereCri='',@WhereCri_CHQ_CLST='',@WhereCri_DEPO='',@WhereCri_ACC='',@WhereCri_AMt=''
	...
	if @CHQDEPO <>'All'
		select @WhereCri_DEPO='  and DepoFlag= '+ char(39) + @CHQDEPO + char(39)
