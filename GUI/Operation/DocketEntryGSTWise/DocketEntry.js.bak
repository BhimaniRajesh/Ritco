...
function submitClick() {
    ...
    var dockettotal = GetGrandTotalValue();
    ...
    if (parseFloat(dockettotal) > 100000) {
...
function calculateMinPercentSubTotal() {
    ...
    if (cmbratetype.value == "W")
        newrate = parseFloat(subtotal) / parseFloat(txttotchargedweight.value);
        //newrate = parseFloat(txtsubtotal.value) / parseFloat(txttotchargedweight.value); /* KK Cess*/
    else if (cmbratetype.value == "P")
        newrate = parseFloat(subtotal) / parseFloat(hdntotnopkgs.value);
        //newrate = parseFloat(txtsubtotal.value) / parseFloat(hdntotnopkgs.value); /* KK Cess*/
    else if (cmbratetype.value == "T")
        newrate = parseFloat(subtotal) / (parseFloat(txttotchargedweight.value) * 1000);
        //newrate = parseFloat(txtsubtotal.value) / (parseFloat(txttotchargedweight.value) * 1000);/* KK Cess*/
    else
        return false;
...
function IsBookDateAllowed(strdockbook, daterule) {

    var dtdockbook = new Date(DateString(strdockbook));

    var arrtmp, arrblwrule, arrabvrule;
    try {
        ...
        var dtblwcompare = new Date(DateString(arrblwrule[1]));
        if (dtdockbook < dtblwcompare)
            return false;

        var dtabvcompare = new Date(DateString(arrabvrule[1]));
        if (dtdockbook > dtabvcompare)
            return false;
    }
    catch (e) {
        return false;
...
function validateStep1() {
    ...
    if (!IsBookDateAllowed(txtdockdate.value, hdnbookdaterule.value))
        return false;
...
function DisableStep1() {
    ...
    var txtto = document.getElementById(ControlIdPrefix + "txtto");
    var btndelpopup = document.getElementById(ControlIdPrefix + "btndelpopup");
    var btnpartypopup = document.getElementById(ControlIdPrefix + "btnpartypopup");
    var trvolumetric = document.getElementById(ControlIdPrefix + "trvolumetric");
    var trlocal = document.getElementById(ControlIdPrefix + "trlocal");
    var trdacc = document.getElementById(ControlIdPrefix + "trdacc");
    var trbtnstep1 = document.getElementById(ControlIdPrefix + "trbtnstep1");
    var divpartycsgne = document.getElementById(ControlIdPrefix + "divpartycsgne");
    var optpartycsgn = document.getElementById(ControlIdPrefix + "optpartycsgn");
    var optpartycsge = document.getElementById(ControlIdPrefix + "optpartycsge");
    var txtcsgncd = document.getElementById(ControlIdPrefix + "txtcsgncd");
    var txtcsgnnm = document.getElementById(ControlIdPrefix + "txtcsgnnm");
    var txtcsgecd = document.getElementById(ControlIdPrefix + "txtcsgecd");
    var txtcsgenm = document.getElementById(ControlIdPrefix + "txtcsgenm");
    var chklocal = document.getElementById(ControlIdPrefix + "chklocal");
    var lblorgnloc = document.getElementById(ControlIdPrefix + "lblorgnloc");

    txtto.disabled = txtdockno.disabled = txtdockdate.disabled = cmbpaybas.disabled = txtpartycode.disabled = txtdelloc.disabled = optentrytype.disabled = true;
    ...
    if (optpartycsgn.checked == true) {
        txtcsgncd.value = txtpartycode.value;
        txtcsgnnm.value = txtpartyname.value;
    }
    else {
        txtcsgecd.value = txtpartycode.value;
        txtcsgenm.value = txtpartyname.value;
    }

    // LOCAL DOCKET SETTINGS 
    if (lblorgnloc.innerText.toUpperCase() == txtdelloc.value.toUpperCase())
        chklocal.disabled = false;
    else {
