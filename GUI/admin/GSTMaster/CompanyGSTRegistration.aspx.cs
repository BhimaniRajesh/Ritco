...
public partial class GUI_admin_GSTMaster_CompanyGSTRegistration : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack && !IsCallback)
        {
            ...
            using (var oDal = new WebXHelper())
            {
                ...
                { dtFormat = oHelper.GetColumnList("CompanyGST"); }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                var oGSTCustomerMasterEntity = new GSTCompanyMasterEntity();

                oGSTCustomerMasterEntity.CompanyCode = hdnCompanyCode.Value;
                ...
                SqlParameter[] prm = {
                                         new SqlParameter("@CompanyHdrDetail", XmlUtility.XmlSerializeToString(oGSTCustomerMasterEntity))
                                     };
                ...
                oHelper.strModuleName = "CompanyGST";
            ...
            else
            {
                ...
                {
                    checkRowData();
                    var oGSTCompanyMasterEntity = new GSTCompanyMasterEntity();

                    oGSTCompanyMasterEntity.CompanyCode = hdnCompanyCode.Value;
                    oGSTCompanyMasterEntity.Address = txtGSTBillingAddres.Text;

                    oGSTCompanyMasterEntity.StateCode = hdnGSTState.Value;
                    oGSTCompanyMasterEntity.CityCode = hdnGSTCity.Value;
                    oGSTCompanyMasterEntity.CGSTNumber = txtCGSTNumber.Text;
                    oGSTCompanyMasterEntity.IGSTNumber = txtIGSTNumber.Text;
                    ...
                    foreach (RepeaterItem grvDet in rptOrderEntry.Items)
                    {
                        ...
                        var txtLocation = (TextBox)grvDet.FindControl("txtLocation");
                        var hdnLocationCode = (HiddenField)grvDet.FindControl("hdnLocationCode");

                        var TRD = new GSTMasterEntity();

                        if (txtStateUTName.Text.Trim() != "")
                        {
                            TRD.StateName = txtStateUTName.Text.Trim();
                            TRD.Address = txtAddress.Text.Trim();
                            TRD.City = txtCity.Text.Trim();
                            //TRD.ProvisionalNumber = txtProvisionalNumber.Text.Trim();
                            TRD.GSTType = lblGSTType.Text.Trim();
                            TRD.GSTINumber = txtGSTTiNumber.Text.Trim();
                            TRD.StateTypeCode = hdnStateTypeCode.Value;
                            TRD.StateCode = hdnStateUTCode.Value;
                            TRD.CityCode = hdnCityCode.Value;
                            TRD.Location = txtLocation.Text.Trim();
                            TRD.LocationCode = hdnLocationCode.Value;
                    ...
                    oGSTCompanyMasterEntity.ListGSTMasterEntity = lst_GSTMasterEntity;
                    var oGSTMasterController = new GSTMasterController();
                    var dt = oGSTMasterController.InsertCompanyGSTDetail(oGSTCompanyMasterEntity);
                    ...
                    {
                        //msgBox.Show("Company GST Registration details saved successfully.");
                        ConsumeToken();
    ...
    protected void fillData()
    {
        ...
        {
            var oGSTMasterController = new GSTMasterController();
            var dt = oGSTMasterController.GetCompanyGSTDetail(hdnCompanyCode.Value);
            ...
            {
                lblCutomerID.Text = dt.Tables[0].Rows[0]["COMPANY_CODE"].ToString() + " : " + dt.Tables[0].Rows[0]["COMPANY_NAME"].ToString();
                //txtGSTState.Text = Convert.ToString(dt.Tables[0].Rows[0]["GSTBillingStateName"]);
                ddlGSTState.SelectedValue = Convert.ToString(dt.Tables[0].Rows[0]["GSTBillingState"]);
                hdnGSTState.Value = Convert.ToString(dt.Tables[0].Rows[0]["GSTBillingState"]);

                txtGSTCity.Text = Convert.ToString(dt.Tables[0].Rows[0]["GSTBillingCityName"]);
                hdnGSTCity.Value = Convert.ToString(dt.Tables[0].Rows[0]["GSTBillingCity"]);

                txtGSTBillingAddres.Text = Convert.ToString(dt.Tables[0].Rows[0]["GSTBillingAddress"]);
                txtCGSTNumber.Text = Convert.ToString(dt.Tables[0].Rows[0]["CSGTnumber"]);
                txtIGSTNumber.Text = Convert.ToString(dt.Tables[0].Rows[0]["IGSTnumber"]);
            }

            if (dt.Tables[1].Rows.Count > 0)
            {
                BindData(dt.Tables[1]);
            }
            else
            {
            ...
            //tdCompanyCriteria.Attributes["disabled"] = "disabled";
    ...
    protected void rptOrderEntry_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            txtGSTTiNumber.Attributes["onblur"] = "return onValidateIGSTNumberExist(this,'COMPANY','" + hdnCompanyCode.Value + "');";
            //txtProvisionalNumber.Attributes["onblur"] = "return onValidateCGSTNumberExist(this,'COMPANY','" + hdnCompanyCode.Value + "');";//commented on 04-jul-2017, to provide one textbox for both Provisional / GSTIN Number.
        ...
        try
        {
            ...
            if (ds.Rows.Count > 0)
            {
                for (int i = 0; i < ds.Rows.Count; i++)
                {
                    ...
                    TRD.StateName = Convert.ToString(ds.Rows[i]["GSTBillingStateName"]);
                    TRD.StateCode = Convert.ToString(ds.Rows[i]["BillingStateCode"]);
                    TRD.Address = Convert.ToString(ds.Rows[i]["BillingAddress"]);
                    TRD.City = Convert.ToString(ds.Rows[i]["GSTBillingCityName"]);
                    TRD.CityCode = Convert.ToString(ds.Rows[i]["BillingCityCode"]);
                    TRD.Location = Convert.ToString(ds.Rows[i]["BillingLocationName"]);
                    TRD.LocationCode = Convert.ToString(ds.Rows[i]["BillingLocationCode"]);
                    //TRD.ProvisionalNumber = Convert.ToString(ds.Rows[i]["ProvisionalNumber"]);
                    TRD.GSTType = Convert.ToString(ds.Rows[i]["GSTINType"]);
                    TRD.GSTINumber = Convert.ToString(ds.Rows[i]["GSTINNumber"]);
                    TRD.StateTypeCode = Convert.ToString(ds.Rows[i]["BillingStateType"]);
    ...
    /// <summary>
    /// Get List For Company List
    /// </summary>
    /// <param name="CompanyCode"></param>
    /// <returns></returns>
    //[WebMethod]
    //public static string CheckCompanyCode(string CompanyCode)
    //{
    //    var oGSTMasterController = new GSTMasterController();
    //    var dtResult = oGSTMasterController.ValidateCompany(CompanyCode);
    ...
    protected void checkRowData()
    {
        ...
        foreach (RepeaterItem grvDet in rptOrderEntry.Items)
        {
            ...
            TRD.Address = txtAddress.Text.Trim();
            TRD.City = txtCity.Text.Trim();
            //TRD.ProvisionalNumber = txtProvisionalNumber.Text.Trim();//commented on 04-jul-2017, to provide one textbox for both Provisional / GSTIN Number.
            TRD.GSTType = lblGSTType.Text.Trim();
            TRD.GSTINumber = txtGSTTiNumber.Text.Trim();
    ...
    protected void lnkFormat_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("CompanyGSTUploadFormat.xls", false);
    }
