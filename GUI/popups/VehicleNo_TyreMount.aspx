<%@ Page Language="C#" AutoEventWireup="true" CodeFile="VehicleNo_TyreMount.aspx.cs"
    ...
    <link id="Link1" href="~/GUI/images/style.css" rel="Stylesheet" type="text/css" runat="server" />
     <script type="text/javascript" language="javascript">
         ...
         function selectVehno(obj) 
         {
            ...
            var hf_Opener_Poscate = document.getElementById("hPosCate").value
            var hf_Opener_TT = document.getElementById("hTT").value 
            ...
            window.opener.document.getElementById(hf_Opener_Poscate).value=returnValue[3];
            window.opener.document.getElementById(hf_Opener_TT).value=returnValue[4];
            window.opener.document.getElementById(hf_Opener_Km).value=returnValue[5];
            window.opener.ConSP();
    ...
    <form id="form1" runat="server">
        <div>
           ...
           <tr>
            ...
            <asp:HiddenField ID="hPosCate" runat="server" Value=""/>
            <asp:HiddenField ID="hTT" runat="server" Value=""/>
                        ...
                        <asp:GridView Width="100%" HeaderStyle-CssClass="dgHeaderStyle" CssClass="dgRowStyle" BorderColor="#8BA0E5"
                            ...
                            <Columns>
                                <asp:TemplateField ShowHeader="False">
                                    <ItemTemplate>
                                        <input type="radio" name="radSelect" onclick="javascript:selectVehno(this);" value='<%#Eval("VEHNO")%>~<%#Eval("VEH_INTERNAL_NO")%>~<%#Eval("VehCategory")%>~<%#Eval("POS_ALLOWED")%>~<%#Eval("TRUCK_TRAILER")%>~<%#Eval("CURRENT_KM_READ")%>' />
                                    </ItemTemplate>
...
(SELECT CODEDESC FROM Webx_Master_General WHERE CODETYPE='VHCAT'AND codeid=p.TRUCK_TRAILER) as VehCategory,p.TRUCK_TRAILER,p.POS_ALLOWED
