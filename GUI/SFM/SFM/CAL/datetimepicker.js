...
//var idxNo;
//var allidxNo
function NewCal(argObj,pCtrl,pFormat,pShowTime,pTimeMode,pScroller,pHideSeconds)
{
	...
	RenderCal(argObj.name);
	winCal.focus();
}

function RenderCal(argObj)
{
	...
	{
		vCalData=vCalData+GenCell(argObj);
	...
	for (j=1;j<=Cal.GetMonDays();j++)
	{
		...
		if ((j==dtToday.getDate())&&(Cal.Month==dtToday.getMonth())&&(Cal.Year==dtToday.getFullYear()))
			strCell=GenCell(argObj,j,true,TodayColor);//Highlight today's date
		else
		{
			if ((j==selDate.getDate())&&(Cal.Month==selDate.getMonth())&&(Cal.Year==selDate.getFullYear()))//modified version 1.7
			{
				strCell=GenCell(argObj,j,true,SelDateColor);
			}
			else
			{	
				if (MondayFirstDay==true)
				{
					if (vDayCount%7==0)
						strCell=GenCell(argObj,j,false,SundayColor);
					else if ((vDayCount+1)%7==0)
						strCell=GenCell(argObj,j,false,SaturdayColor);
					else
						strCell=GenCell(argObj,j,null,WeekDayColor);					
				} 
				else
				{
					if (vDayCount%7==0)
...
//	eval('document.form1.demo15'+[allidxNo]).focus();
}

function GenCell(argObj,pValue,pHighLight,pColor)//Generate table cell with value
{
	...
	if (Cal.ShowTime)
	{
		vTimeStr="eval('window.opener.form1.'" + "+'" + argObj + "').value+=' '+"+"winMain.Cal.getShowHour()"+"+':'+"+"winMain.Cal.Minutes";
		if (Cal.ShowSeconds)
		...
		vTimeStr="";		
	//PCellStr="<td "+vColor+" width="+CellWidth+" align='center'><font face='verdana' size='2'"+vHLstr1+"<a href=\"javascript:eval('window.opener.form1.'" + "+'" + argObj + "').value='" + Cal.FormatDate(PValue) +"';eval('window.opener.form1.'" + "+'" + argObj + "').value='"+Cal.FormatDate(PValue)+"';"+vTimeStr+";eval('window.opener.form1.'" + "+'" + argObj + "').focus();window.close();\">"+PValue+"</a>"+vHLstr2+"</font></td>";
//	alert("first Array" +argObj)
//	var change=argObj
//	var ArrChange=change.split(':')
//	var ArrOne,ArrTwo,ArrThree;
//	
//	ArrOne=ArrChange[0];
//	ArrTwo=ArrChange[1];
//	ArrThree=ArrChange[2];
//	argObj = null
//	argObj=ArrChange[0]+"$"+ArrChange[1]+"$"+ArrChange[2]
//	
//	alert("last array" +argObj)
	
	PCellStr="<td "+vColor+" width="+CellWidth+" align='center'><font face='verdana' size='2'"+vHLstr1+"<a href=\"javascript:eval('window.opener.form1.'" + "+'" + argObj + "').value='"+Cal.FormatDate(PValue)+"';"+vTimeStr+";eval('window.opener.form1.'" + "+'" + argObj + "').focus();window.close();\">"+PValue+"</a>"+vHLstr2+"</font></td>";
	//alert(PCellStr)
	return PCellStr;
}
